document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderDialog(name) {
    ...
    chatScroll.innerHTML = msgs.map(m => {
      ...
      const showAuthor = isOut && firstOut;
      if (isOut) firstOut = false;
  ...
  // ── Inbox view data ────────────────────────────────────
  const CH = {
    livechat: `<svg viewBox="0 0 10 10" fill="none"><path d="M1 2.5C1 1.67 1.67 1 2.5 1h5C8.33 1 9 1.67 9 2.5v3C9 6.33 8.33 7 7.5 7H5.5L3 9V7H2.5C1.67 7 1 6.33 1 5.5v-3z" fill="#010103" opacity=".6"/></svg>`,
    whatsapp: `<svg viewBox="0 0 10 10" fill="none"><circle cx="5" cy="5" r="4" fill="#25D366"/><path d="M7 6.2c-.1-.1-.7-.35-.8-.38-.1-.03-.18 0-.25.1l-.3.4c-.07.1-.15.1-.27.05-.6-.28-1-.6-1.35-1.1-.1-.15-.02-.23.07-.3l.25-.28c.07-.1.1-.18.03-.28L4.1 3.8c-.07-.1-.15-.1-.23-.07-.8.38-.82.75-.7 1.1.6 1.5 1.7 2.2 3.3 2.5.35.07.6-.1.65-.45.03-.2.02-.35-.12-.68z" fill="white"/></svg>`,
    android: `<svg viewBox="0 0 10 10" fill="none"><path d="M2 6.5V4.5C2 3.12 3.12 2 4.5 2h1C6.88 2 8 3.12 8 4.5v2c0 .28-.22.5-.5.5h-5C2.22 7 2 6.78 2 6.5zM3.5 1l.5.8M6.5 1L6 1.8" stroke="#3ddc84" stroke-width=".8" stroke-linecap="round"/><rect x="1.5" y="7" width="7" height=".7" rx=".35" fill="#3ddc84"/></svg>`
  };
  ...
  const VIEW_LABELS = { assigned:'Assigned to me', mentions:'Mentions', unassigned:'Unassigned', snoozed:'Snoozed', closed:'Closed', all:'All' };
